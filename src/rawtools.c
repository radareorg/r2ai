...
static char *build_rawtools_prompt(const R2AI_Tools *tools) {
	...
	for (int i = 0; i < tools->n_tools; i++) {
		...
		buffer_size += strlen(tool->name ? tool->name : "") + 10;
		if (tool->description) buffer_size += strlen(tool->description) + 15;
		if (tool->parameters) buffer_size += strlen(tool->parameters) + 15;
		buffer_size += 50; // Extra space for formatting
	}
	...
	buffer[0] = '\0';
	// AITOOD: Use RStrBuf instead!!
	strcat(buffer, "You have access to the following tools:\n\n");

	for (int i = 0; i < tools->n_tools; i++) {
		const R2AI_Tool *tool = &tools->tools[i];
		strcat(buffer, "TOOL: ");
		if (tool->name) strcat(buffer, tool->name);
		strcat(buffer, "\n");
		if (tool->description) {
			strcat(buffer, "DESCRIPTION: ");
			strcat(buffer, tool->description);
			strcat(buffer, "\n");
		}
		if (tool->parameters) {
			strcat(buffer, "PARAMETERS: ");
			strcat(buffer, tool->parameters);
			strcat(buffer, "\n");
		}
		strcat(buffer, "\n");
	}

	strcat(buffer, "To use a tool, respond with exactly one line in this format:\n");
	strcat(buffer, "TOOL: tool_name arguments\n\n");
	strcat(buffer, "Where arguments is a JSON string with the tool parameters.\n");
	strcat(buffer, "Do not include any other text or explanations in your response when using a tool.\n\n");
