...
static void append_headers_to_cmd(RStrBuf *cmd, const char **headers) {	r_strbuf_appendf (cmd, "$headers=@{");
	if (!headers) {
	...
	for (int i = 0; headers[i]; i++) {
		...
		*colon = '\0';
		...
		char *escaped_key = key ? escape_single_quotes (key) : NULL;
		char *escaped_value = value ? escape_single_quotes (value) : NULL;
...
static HttpResponse parse_powershell_response(char *full_response) {	if (!full_response) {
 ...
 */
HttpResponse windows_http_post(const HTTPRequest *request) {	const char *url = request->url;
 ...
 */
HttpResponse windows_http_get(const HTTPRequest *request) {	const char *url = request->url;
	const char **headers = request->headers;
	int timeout = request->config.timeout;
	if (!url) {
		return (HttpResponse){ .code = -1 };
